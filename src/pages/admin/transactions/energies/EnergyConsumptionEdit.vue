<template>
  <div class="p-4">
    <h2 class="text-xl font-bold mb-4">{{ $t('Edit Energy Consumption Record') }}</h2>
...
export default defineComponent({
  name: 'EnergyConsumptionEdit',
  props: {
    energyConsumption: {
      type: Object as () => EnergyConsumption,
      required: true,
    },
  },
  emits: {
  ...
  data() {
    // Ensure date fields are formatted for input type="date" (YYYY-MM-DD)
    const consumptionDate = this.energyConsumption.consumption_date
      ? format(new Date(this.energyConsumption.consumption_date), 'yyyy-MM-dd')
      : ''
    const dueDate = this.energyConsumption.due_date
      ? format(new Date(this.energyConsumption.due_date), 'yyyy-MM-dd')
      : ''
    return {
      form: {
        property_id: this.energyConsumption.property_id as number | null,
        consumption_date: consumptionDate,
        electricity_usage: this.energyConsumption.electricity_usage as number | null,
        gas_usage: this.energyConsumption.gas_usage as number | null,
        water_usage: this.energyConsumption.water_usage as number | null,
        total_cost: this.energyConsumption.total_cost as number | null,
        status: this.energyConsumption.status as string,
        due_date: dueDate,
      } as FormData,
      errors: {
  ...
  methods: {
    async submitForm() {
      // Validate the energyConsumption ID
      const id = this.energyConsumption.id
      if (!Number.isInteger(id) || id <= 0) {
        console.error('Invalid energy consumption ID:', id)
        Swal.fire({
          title: 'Error!',
          text: 'Invalid energy consumption record ID. Please select a valid record.',
          icon: 'error',
          position: 'top-end',
          toast: true,
      ...
      try {
        ...
        const energyConsumptionStore = useEnergyConsumptionStore()
        await energyConsumptionStore.updateEnergyConsumption(payload, id)
        Swal.fire({
          title: 'Success!',
        ...
        let errorMessage = error.response?.data?.message || 'Failed to update energy consumption record.'
