<template>
  <div class="p-4">
    <h2 class="text-xl font-bold mb-4">{{ $t('Edit Client') }}</h2>
...
import { FormData, Errors, Client, Payload } from '../../../../types/client'
  ...
  methods: {
    async submitForm() {
      ...
      try {
        ...
        await makeRequest({
          url: `${import.meta.env.VITE_APP_API_BASE_URL}/v1/clients/${this.client.id}`,
          method: 'put',
          headers: {
        ...
        let errorMessage = error.response?.data?.message || 'Failed to update client.'
