<template>
  <div class="card">
    ...
    <div v-if="payments && payments.length > 0" class="flex justify-between items-center mt-4">
      <div>
        Showing {{ pagination.from }} to {{ pagination.to }} of {{ pagination.total }} pending payments
      </div>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      componentKey: 0,
      statusOptions: ['received', 'rejected'],
      perPageOptions: [
        { value: 10, text: '10' },
        { value: 15, text: '15' },
        { value: 25, text: '25' },
  ...
  async mounted() {
    console.log('Payments mounted, fetching pending payments for user_id:', this.userId, 'lease_id:', this.leaseId);
    ...
    async getPayments(params: { page?: number; per_page?: number; search?: string } = {}) {
      ...
      try {
        ...
        if (response.status === 200) {
          // Filter payments to only include those with 'pending' status
          const pendingPayments = response.data.data.filter((payment: any) => payment.status === 'pending');
          this.payments = pendingPayments.map((payment: any, index: number) => {
          ...
          this.pagination = {
            ...
            last_page: Math.ceil(pendingPayments.length / (params.per_page || 15)),
            from: pendingPayments.length > 0 ? (response.data.current_page - 1) * (params.per_page || 15) + 1 : 0,
            to: Math.min(response.data.current_page * (params.per_page || 15), pendingPayments.length),
          };
          ...
          Swal.fire({
            title: 'Error!',
            text: response.data?.message || 'Failed to fetch pending payments.',
            icon: 'error',
            position: 'top-end',
            toast: true,
            showConfirmButton: false,
        ...
        Swal.fire({
          title: 'Error!',
          text: error.response?.data?.message || error.message || 'Failed to fetch pending payments.',
          icon: 'error',
          position: 'top-end',
          toast: true,
          showConfirmButton: false,
    ...
    async updateStatus(payment: Payment, newStatus: string) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // Refresh the payments list since the updated payment should no longer appear
          await this.getPayments({ page: this.pagination.current_page, per_page: this.pagination.per_page, search: this.searchQuery });
          this.componentKey += 1;
        } else {
          Swal.fire({
            title: 'Error!',
