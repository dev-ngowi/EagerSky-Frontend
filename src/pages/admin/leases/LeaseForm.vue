<template>
  <div class="p-4">
    <h2 class="text-xl font-bold mb-4">{{ $t('Add New Lease', 'Add New Lease') }}</h2>
...
import type { FormData, Errors, Payload, PropertyOption, UserOption, SignedOption } from '../../../types/lease';
...
export default defineComponent({
  ...
  async mounted() {
    console.log('LeaseForm mounted, loading dropdowns');
    try {
    ...
    async submitForm() {
      ...
      try {
        ...
        if (response.status === 201) {
          ...
          throw new Error(response.data?.message || 'Failed to add lease.');
        }
      } catch (error: any) {
        console.error('Submission error:', error.response?.data || error.message);
        let errorMessage = error.response?.data?.message || 'Failed to add lease.';
