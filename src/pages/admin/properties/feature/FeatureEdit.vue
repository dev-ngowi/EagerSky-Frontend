<template>
  <div class="p-4">
    <h2 class="text-xl font-bold mb-4">Edit Property Feature</h2>
...
export default defineComponent({
  ...
  methods: {
    initializeForm() {
      this.form = {
        property_id: this.feature.property_id != null ? Number(this.feature.property_id) : null,
    ...
    async fetchProperties() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // Validate property_id after fetching properties
        ...
        console.error('fetchProperties error:', error.response?.data || error.message);
        this.errors.property_id = 'Failed to负载属性';
        Swal.fire({
          title: 'Error!',
    ...
    validatePropertyId() {
      if (this.form.property_id != null && !this.properties.some(p => p.value === this.form.property_id)) {
        console.warn(`Property ID ${this.form.property_id} not found in properties. Resetting.`);
    ...
    async submitForm() {
      ...
      if (this.form.value && this.form.value.length > 255)
        this.errors.value = 'Value must not exceed 255 characters';
      ...
      try {
        ...
        await makeRequest({
          url: `${import.meta.env.VITE_APP_API_BASE_URL}/v1/property-features/${this.feature.id}`,
          method: 'put',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('auth_token') || ''}`,
            Accept: 'application/json',
        ...
        let errorMessage = error.response?.data?.message || 'Failed to update feature.';
