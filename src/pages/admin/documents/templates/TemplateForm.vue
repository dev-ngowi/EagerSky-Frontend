...
import { FormData, Errors, Payload } from '../../../../types/template'
...
export default defineComponent({
  ...
  methods: {
    ...mapActions(useTemplateStore, ['addTemplate']),
    async handleSubmit() {
      ...
      try {
        ...
        let errorMessage = err.response?.data?.message || 'Failed to add template.'
    ...
    resetForm() {
      Object.assign(this.form, { name: '', content: '' })
      Object.keys(this.errors).forEach((key) => (this.errors[key as keyof Errors] = ''))
      this.$emit('close')
    },
  },
