  ...
  <div class="bg-white shadow-md rounded-lg p-4">
    <template v-if="isLoading">
      <div class="loadingSpiner">
        <Loader :loading-text="'Loading maintenance requests...'" />
      </div>
    </template>
    <template v-else-if="errorMessage">
      <div class="text-center py-4 text-red-600">
        {{ errorMessage }}
        <button class="ml-4 text-blue-600 underline" @click="retryFetch" aria-label="Retry fetching maintenance requests">
          Retry
        </button>
      </div>
    </template>
    <template v-else>
      <div class="filters-container flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
          <VaInput
            v-model="searchQuery"
            placeholder="Search requests by property or status"
      ...
      <div v-if="maintenanceRequests.length === 0" class="text-center py-4 text-gray-500">
      ...
      >
        ...
        <template #cell(room_number)="{ rowData }">
          {{ rowData.room_number || 'N/A' }}
        </template>
        <template #cell(description)="{ rowData }">
          {{ rowData.description || 'N/A' }}
        </template>
        <template #cell(contractor_name)="{ rowData }">
          {{ rowData.contractor_name || 'Awaiting Assignment' }}
        </template>
      ...
      <div v-if="maintenanceRequests.length > 0" class="pagination-section flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 gap-4">
        <div class="text-sm">
...
const getStatusClass = (status: string): string => {
  ...
  const statusClasses: { [key: string]: string } = {
    ...
    'in_progress': 'bg-blue-100 text-blue-800 px-2 py-1 rounded text-sm font-medium',
    rejected: 'bg-red-100 text-red-800 px-2 py-1 rounded text-sm font-medium',
  };
  
  return statusClasses[normalizedStatus] || 'bg-gray-100 text-gray-800 px-2 py-1 rounded text-sm font-medium';
};

const fetchMaintenanceRequests = async (params: { page?: number; per_page?: number; search?: string } = {}) => {
  ...
  try {
    const baseUrl = `${import.meta.env.VITE_APP_API_BASE_URL}/v1/tenant/maintenance-requests`;
    ...
    pagination.value = response.data.pagination || {
      ...
      last_page: Math.ceil(requestData.length / (params.per_page || 10)) || 1,
      from: requestData.length > 0 ? (params.page || 1 - 1) * (params.per_page || 10) + 1 : 0,
      to: Math.min((params.page || 1) * (params.per_page || 10), requestData.length),
    };

    if (maintenanceRequests.value.length === 0 && !searchQuery.value && !params.search) {
      Swal.fire({
        title: 'Info',
    ...
    let errorMessageText = 'Failed to fetch maintenance requests.';
...
@media (max-width: 640px) {
  ...
  .table-responsive :deep(th[data-key="description"]),
  .table-responsive :deep(td[data-key="description"]) {
    min-width: 100px;
  }

  /* Reduce visibility of less critical columns on mobile */
  .table-responsive :deep(th[data-key="room_number"]),
  .table-responsive :deep(td[data-key="room_number"]),
  .table-responsive :deep(th[data-key="contractor_name"]),
  .table-responsive :deep(td[data-key="contractor_name"]),
  .table-responsive :deep(th[data-key="created_at"]),
  .table-responsive :deep(td[data-key="created_at"]) {
