<template>
  <div class="form-container">
    ...
    <div class="form-header">
      <h1 class="form-title">🏡 Rental Application</h1>
      <p class="form-subtitle">Complete your rental journey in 7 simple steps</p>
    </div>

    <!-- Progress Container -->
    <div class="steps-container">
      <div class="progress-bar relative flex justify-between items-center mb-6">
        <div class="progress-line absolute top-1/2 -translate-y-1/2 w-full h-1.5 bg-gray-200 rounded-full">
          <div class="progress-line-active h-1.5 bg-blue-600 rounded-full transition-all duration-300" :style="{ width: progressPercent + '%' }"></div>
        </div>
        <div
          v-for="step in totalSteps"
          :key="step"
          class="step-indicator relative z-10 w-10 h-10 flex items-center justify-center rounded-full text-white font-semibold cursor-pointer transition-all duration-200 text-sm"
          :class="{ 
            'bg-blue-600': step === currentStep, 
            'bg-green-500': completedSteps.includes(step),
            'bg-gray-300': !canAccessStep(step),
            'bg-blue-200': canAccessStep(step) && step !== currentStep && !completedSteps.includes(step)
          }"
      ...
      <div class="step-labels grid grid-cols-7 gap-2 text-center text-xs font-medium">
        <div
          ...
          :class="{ 
            ...
            'text-gray-400': !canAccessStep(index + 1)
          }"
        >
          {{ label }}
        </div>
    ...
    <!-- Progress Statistics -->
    <div class="progress-stats bg-white rounded-lg shadow p-4 mb-6 flex flex-col sm:flex-row sm:justify-between gap-4" v-if="completedSteps.length > 0">
      <div class="stats-item">
        <span class="stats-label text-gray-600 font-medium">Completed Steps:</span>
        <span class="stats-value font-semibold text-gray-900 ml-2">{{ completedSteps.length }}/{{ totalSteps }}</span>
      </div>
      <div class="stats-item">
        <span class="stats-label text-gray-600 font-medium">Current Progress:</span>
        <span class="stats-value font-semibold text-gray-900 ml-2">{{ Math.round((completedSteps.length / totalSteps) * 100) }}%</span>
      </div>
    </div>
    ...
    <div class="property-form">
      ...
      <div v-if="currentStep === 1" class="step-content">
        <div class="step-header">
          <h3 class="step-title">Select Booking</h3>
          <p class="step-description">Choose your property booking to start the application process</p>
        </div>

        <div class="form-grid">
          <div class="form-group">
            <label for="booking" class="block text-sm font-medium text-gray-700 mb-2">Property Booking</label>
            <select
              v-model="formData.bookingId"
              :disabled="loading || completedSteps.includes(1)"
              :class="{ 'is-invalid': errors.bookingId }"
              class="form-select w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
              required
              aria-label="Select a property booking"
            >
            ...
            <div v-if="errors.bookingId" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.bookingId }}</div>
          </div>
        </div>

        <div class="info-card bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6 rounded-lg">
          <h3 class="text-lg font-semibold text-yellow-800 flex items-center gap-2">
            <span>⚠️</span> Important Notice
          </h3>
          <p class="text-gray-700 mt-2">Once you start the application process, you cannot return to previous steps. Please ensure all information is ready before proceeding.</p>
          <div class="completion-status mt-4" v-if="completedSteps.includes(1)">
            <span class="completed-badge inline-flex items-center px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
              ✓ Step Completed
            </span>
            <small class="text-gray-500 block mt-1">Completed on {{ getCompletionDate(1) }}</small>
          </div>
        </div>

        <div class="info-card bg-blue-50 border-l-4 border-blue-400 p-4 rounded-lg">
          <h3 class="text-lg font-semibold text-blue-800 flex items-center gap-2">
            <span>📋</span> What happens next?
          </h3>
          <p class="text-gray-700 mt-2">After selecting your booking, you'll complete a rental application form with your personal and employment details.</p>
        </div>

        <div v-if="loading" class="loading-indicator flex items-center gap-2 mt-4">
      ...
      <div v-if="currentStep === 2" class="step-content">
        <div class="step-header">
          <h3 class="step-title">Rental Application Details</h3>
          <p class="step-description">Provide your personal and employment information for the rental application</p>
          <div class="completion-status mt-2" v-if="completedSteps.includes(2)">
            <span class="completed-badge inline-flex items-center px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
              ✓ Step Completed
            </span>
            <small class="text-gray-500 block mt-1">Completed on {{ getCompletionDate(2) }}</small>
          </div>
        </div>
        ...
        <div v-if="!isApplicationPending">
          <div class="form-grid">
            <div class="form-group">
              ...
              <div v-if="errors.branch_id" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.branch_id }}</div>
            </div>

            <div class="form-group">
              <label for="employment" class="block text-sm font-medium text-gray-700 mb-2">Employment Status</label>
              <select
                v-model="formData.application.employment_status"
                @change="onEmploymentStatusChange"
                :disabled="loading || completedSteps.includes(2)"
                :class="{ 'is-invalid': errors.employment_status }"
                class="form-select w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
                required
              ...
              <div v-if="errors.employment_status" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.employment_status }}</div>
            </div>

            <div class="form-group" v-if="formData.application.employment_status === 'student'">
              ...
              <input
                ...
                class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
                required
                aria-label="Enter registration number"
              />
              <div v-if="errors.registration_number" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.registration_number }}</div>
            </div>

            <div v-if="formData.application.employment_status === 'student'" class="form-group-placeholder"></div>

            <div class="form-group">
              <label for="nida" class="block text-sm font-medium text-gray-700 mb-2">NIDA Number</label>
              <input
                type="text"
                v-model="formData.application.nida_number"
                placeholder="Enter your 20-digit NIDA number (YYYY...)"
                :disabled="loading || completedSteps.includes(2)"
                :class="{ 'is-invalid': errors.nida_number }"
                class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
                maxlength="20"
                required
                aria-label="Enter NIDA number"
                @blur="validateNidaNumber"
              />
              <div v-if="errors.nida_number" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.nida_number }}</div>
            </div>

            <div class="form-group">
              <label for="income" class="block text-sm font-medium text-gray-700 mb-2">Annual Income (TZS)</label>
              <select
                v-model="formData.application.annual_income"
                :disabled="loading || completedSteps.includes(2)"
                :class="{ 'is-invalid': errors.annual_income }"
                class="form-select w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
                required
                aria-label="Select annual income range"
              >
              ...
              <div v-if="errors.annual_income" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.annual_income }}</div>
            </div>

            <div class="form-group">
              <label for="background" class="block text-sm font-medium text-gray-700 mb-2">Background Check Status</label>
              <select
                v-model="formData.application.background_check_status"
                :disabled="loading || completedSteps.includes(2)"
                :class="{ 'is-invalid': errors.background_check_status }"
                class="form-select w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
                required
                aria-label="Select background check status"
              >
                <option value="">Select status...</option>
              ...
              <div v-if="errors.background_check_status" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.background_check_status }}</div>
            </div>

            <div class="form-group">
              <label for="credit" class="block text-sm font-medium text-gray-700 mb-2">Credit Report Status</label>
              <select
                v-model="formData.application.credit_report_status"
                :disabled="loading || completedSteps.includes(2)"
                :class="{ 'is-invalid': errors.credit_report_status }"
                class="form-select w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 transition-all duration-200"
                required
                aria-label="Select credit report status"
              >
                <option value="">Select status...</option>
                <option value="good">Good</option>
                <option value="bad">Bad</option>
              ...
              <div v-if="errors.credit_report_status" class="invalid-feedback text-red-500 text-sm mt-1">{{ errors.credit_report_status }}</div>
            </div>
          </div>
        </div>
        <div v-else class="info-card bg-blue-50 border-l-4 border-blue-400 p-4 rounded-lg">
          <h3 class="text-lg font-semibold text-blue-800 flex items-center gap-2">
            <span>⏳</span> Waiting for Approval
          </h3>
          <p class="text-gray-700 mt-2">Your rental application has been submitted and is pending approval. You'll be notified once it's reviewed.</p>
          <div class="completion-status mt-4" v-if="completedSteps.includes(2)">
            <span class="pending-badge inline-flex items-center px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm font-medium">
              ⏳ Application Submitted
            </span>
            <small class="text-gray-500 block mt-1">Submitted on {{ getCompletionDate(2) }}</small>
          </div>
        </div>
      </div>
      ...
      <div v-if="currentStep === 3" class="step-content">
        <div class="step-header">
          <div class="completion-status mt-2" v-if="completedSteps.includes(3)">
            <span class="completed-badge inline-flex items-center px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
              ✓ Step Completed
            </span>
            <small class="text-gray-500 block mt-1">Completed on {{ getCompletionDate(3) }}</small>
          </div>
        </div>
        <LeaseCreator
          ref="leaseCreator"
      ...
      <div v-if="currentStep === 4" class="step-content" role="region" aria-labelledby="sign-lease-title">
        <div class="step-header">
          <div class="completion-status" v-if="completedSteps.includes(4)">
            ...
            <small class="text-gray-500 block mt-1">Completed on {{ getCompletionDate(4) }}</small>
          </div>
        </div>
        <LeaseSigner
          ref="leaseSigner"
          ...
          @session-expired="handleSessionExpired"
        />
        <div v-if="errors.lease_signature" class="invalid-feedback text-red-500 text-sm mt-4" role="alert">
      ...
      <div v-if="currentStep === 5" class="step-content">
        <div class="step-header">
          <div class="completion-status" v-if="completedSteps.includes(5)">
            ...
            <small class="text-gray-500 block mt-1">Completed on {{ getCompletionDate(5) }}</small>
          </div>
        </div>
        <BillingForm
          ref="billingForm"
      ...
      <div v-if="currentStep === 6" class="step-content">
        <div class="step-header">
          <div class="completion-status" v-if="completedSteps.includes(6)">
            ...
            <small class="text-gray-500 block mt-1">Completed on {{ getCompletionDate(6) }}</small>
          </div>
        </div>
        ...
        <div v-if="isPaymentPending && formData.payment.status !== 'received'">
          <div class="info-card bg-blue-50 border-l-4 border-blue-400 p-4 rounded-lg">
            ...
            <p class="text-gray-700 mt-2">Your payment has been submitted and is pending approval. You'll be notified once it's reviewed.</p>
            <div class="completion-status mt-4">
              <span class="pending-badge inline-flex items-center px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm font-medium">
                ⏳ Payment Submitted - Awaiting Verification
              </span>
              <small class="text-gray-500 block mt-1">Submitted on {{ getCompletionDate(6) }}</small>
            </div>
          </div>
        </div>
        ...
        <div v-else>
          ...
          />
          <div v-if="errors.billing_address_id" class="invalid-feedback text-red-500 text-sm mt-4">{{ errors.billing_address_id }}</div>
          <div v-if="errors.lease_term_period" class="invalid-feedback text-red-500 text-sm mt-4">{{ errors.lease_term_period }}</div>
        </div>
      </div>
      ...
      <div v-if="currentStep === 7" class="step-content">
        <div class="step-header">
          <h3 class="step-title">Application Complete</h3>
          <p class="step-description">Review and finalize your application submission</p>
        </div>

        <div class="review-section">
          <div class="review-card">
            <h4 class="review-card-title">Process Summary</h4>
            <div class="review-grid">
              ...
              >
                <span class="review-label">{{ stepLabels[step - 1] }}:</span>
                <span class="review-value">
                  <span v-if="completedSteps.includes(step)" class="inline-flex items-center px-2 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                    ✓ Completed
                  </span>
                  <span v-else class="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-600 rounded-full text-sm font-medium">
                    ⏳ Pending
                  </span>
                  <span v-if="completedSteps.includes(step)" class="ml-2 text-gray-500 text-sm">{{ getCompletionDate(step) }}</span>
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="info-card bg-blue-50 border-l-4 border-blue-400 p-4 rounded-lg">
            <h3 class="text-lg font-semibold text-blue-800 flex items-center gap-2">
              <span>✅</span> Complete Submission
            </h3>
            <p class="text-gray-700 mt-2">Finalize your application by completing the submission.</p>
            <button 
              class="action-button submit-button mt-4 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg font-medium hover:from-green-600 hover:to-emerald-700 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
              @click="completePayment" 
              :disabled="loading"
              aria-label="Complete submission"
            >
      ...
      <div class="form-actions" v-if="currentStep < 7">
        ...
        <button
          class="action-button px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg font-medium hover:from-blue-600 hover:to-indigo-700 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
