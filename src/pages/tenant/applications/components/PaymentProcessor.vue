...
interface Payment {
  ...
  receipt_number?: string | null; // Fixed: allow null
  payment_id?: number | string | null; // Fixed: added payment_id
}
...
export default defineComponent({
  ...
  props: {
    payment: {
      ...
      default: () => ({
        ...
        status: undefined,
        receipt_url: null,
        receipt_number: null, // Fixed: now valid
        payment_id: null, // Fixed: now valid
      }),
    },
    selectedTermPeriod: {
  ...
  setup(props, { emit }) {
    ...
    const paymentId = ref<number | string | null>(null); // Fixed: allow string
    ...
    const localPayment = ref<Payment>({
      ...
      receipt_number: props.payment.receipt_number ?? null, // Fixed: now valid
      payment_id: props.payment.payment_id ?? null, // Fixed: now valid
    });
    ...
    onMounted(async () => {
      ...
      try {
        ...
        if (props.payment.status === 'completed' || props.payment.status === 'received' || props.payment.status === 'approved') {
          ...
          // Fixed: Use nullish coalescing operator
          localPayment.value.receipt_number = receiptNumber.value ?? null;
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        // Fixed: Ensure paymentId is set
        if (!paymentId.value) {
          paymentId.value = `temp_${uuidv4()}`;
        }
        emit('payment-failed', {
          error,
          paymentId: paymentId.value,
    ...
    const generateReceipt = async () => {
      ...
      try {
        ...
        localPayment.value.receipt_url = receiptUrl.value;
        // Fixed: Use nullish coalescing operator
        localPayment.value.receipt_number = receiptNum ?? null;
