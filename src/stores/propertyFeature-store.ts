...
export const usePropertyFeatureStore = defineStore('propertyFeature', {
  ...
  actions: {
    async getPropertyFeatures(
      params: { page?: number; per_page?: number; search?: string; property_id?: number } = {},
    ) {
      ...
      try {
        ...
        if (response.status === 200) {
          this.propertyFeatures = response.data.data.map((feature: any) => ({
            ...
            value: feature.value,
            created_at: feature.created_at ? format(new Date(feature.created_at), 'd MMMM yyyy') : 'None',
            updated_at: feature.updated_at ? format(new Date(feature.updated_at), 'd MMMM yyyy') : 'None',
          }))
    ...
    async addPropertyFeature(payload: any) {
      ...
      try {
        ...
        Swal.fire({
          title: 'Error!',
          text: error.response?.data?.message || 'Failed to add property feature.',
          icon: 'error',
          position: 'top-end',
          toast: true,
          showConfirmButton: false,
    ...
    async updatePropertyFeature(payload: any) {
      ...
      try {
        ...
        Swal.fire({
          title: 'Error!',
          text: error.response?.data?.message || 'Failed to update property feature.',
          icon: 'error',
          position: 'top-end',
          toast: true,
          showConfirmButton: false,
    ...
    async deletePropertyFeature(id: number) {
      try {
        ...
        Swal.fire({
          title: 'Error!',
          text: error.response?.data?.message || 'Failed to delete property feature.',
          icon: 'error',
          position: 'top-end',
          toast: true,
          showConfirmButton: false,
